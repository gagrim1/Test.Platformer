...
Блин, проще рассказать, чем написать. В основном:

* удалил всё, что связано с healthSystem, ловушку, и UI
* написал базовую версию GameManager с данными об игре
* написал InputSystem
* попробовал разделить Player и MovementScript на Jump и Walk Controllerы, но пока плохо.

Детали при беседе.

# Это всё уже не очень актуально, но пусть будет

Вроде бы я доделал проект до прошлого состояния по функционалу.
А что-то писать ещё лень.